...
export default function PMDocumentsPage() {
    ...
    const fetchDocuments = async () => {
        try {
            setIsLoading(true)
            const response = await api.get("/project-manager/documents/hierarchy")
    ...
    const handleDownload = async (doc: Document) => {
        try {
            const response = await api.get(`/project-manager/documents/${doc.id}/download`, {
                responseType: 'blob'
            })
    ...
    const handleDelete = async (docId: string) => {
        ...
        try {
            const response = await api.delete(`/project-manager/documents/${docId}`)
            if (response.data.success) {
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            const response = await api.post('/project-manager/documents/upload-self', formData, {
                headers: {
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground">
                        All documents organized by role hierarchy
                    </p>
                </div>
