...
export default function TeamMembersPage() {
    ...
    const handleReactivate = async (id: string) => {
        if (!confirm("Are you sure you want to reactivate this team member?")) {
            return
        }

        try {
            await api.put(`/admin/team-members/${id}`, { isActive: true })
            fetchTeamMembers()
            alert("Team member reactivated successfully")
        } catch (error) {
    ...
    const handlePermanentDelete = async (id: string) => {
        if (
            !confirm(
                "⚠️ WARNING: This will PERMANENTLY delete this team member and ALL associated data. This action CANNOT be undone. Are you absolutely sure?"
            )
        ) {
            return
        }
                        ...
                        <TableBody>
                            {isLoading ? (
                                ...
                                filteredTeamMembers.map((member) => (
                                    <TableRow key={member.id} className={!member.isActive ? "opacity-60 bg-gray-50 dark:bg-gray-900/50" : ""}>
                                            ...
                                            <DropdownMenu>
                                                ...
                                                <DropdownMenuContent align="end">
                                                    <DropdownMenuItem asChild>
                                                        <Link href={`/admin/team-members/${member.id}`} className="cursor-pointer">
                                                            <Eye className="h-4 w-4 mr-2" />
                                                            View Details
                                                        </Link>
                                                    </DropdownMenuItem>
                                                    <DropdownMenuItem asChild>
                                                        <Link href={`/admin/team-members/${member.id}/edit`} className="cursor-pointer">
                                                            <Edit className="h-4 w-4 mr-2" />
                                                            Edit
                                                        </Link>
                                                    </DropdownMenuItem>
                                                    <DropdownMenuItem asChild>
                                                        <Link href={`/admin/team-members/${member.id}/assign-clients`} className="cursor-pointer">
