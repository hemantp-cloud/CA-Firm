...
export default function ProjectManagersPage() {
  ...
  useEffect(() => {
    const fetchPMs = async () => {
      ...
      try {
        ...
        const apiEndpoint = session.user?.role === 'SUPER_ADMIN'
          ? "http://localhost:4000/api/super-admin/users?role=PROJECT_MANAGER"
          : "http://localhost:4000/api/admin/users?role=PROJECT_MANAGER"
        ...
        if (response.ok) {
          ...
          console.error("Failed to fetch project managers:", response.status)
        }
      } catch (error) {
  ...
  const getNewLink = () => {
    return session?.user?.role === 'SUPER_ADMIN'
      ? "/super-admin/project-managers/new"
      : "/admin/project-managers/new"
  }

  const getEditLink = (id: string) => {
    return session?.user?.role === 'SUPER_ADMIN'
      ? `/super-admin/project-managers/${id}/edit`
            ...
            <tbody className="divide-y divide-gray-100">
              {pms.map((pm) => (
                <tr key={pm.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">
                    <Link href={getEditLink(pm.id)} className="text-emerald-600 hover:text-emerald-800 text-sm font-medium">
                      Edit
                    </Link>
                  </td>
                </tr>
