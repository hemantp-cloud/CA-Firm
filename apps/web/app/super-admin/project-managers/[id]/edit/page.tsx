...
export default function EditProjectManagerPage() {
    ...
    const fetchPM = async () => {
        try {
            ...
            const response = await api.get(`/super-admin/users/PROJECT_MANAGER/${id}`)
            ...
            setError(err.response?.data?.message || "Failed to fetch project manager")
        } finally {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await api.put(`/super-admin/users/PROJECT_MANAGER/${id}`, formData)

            if (response.data.success) {
                alert("Project Manager updated successfully!")
                router.push("/super-admin/project-managers")
            } else {
                setError(response.data.message || "Failed to update project manager")
            }
        } catch (err: any) {
            setError(err.response?.data?.message || "Failed to update project manager")
        } finally {
            setLoading(false)
        }
    }
    ...
    const handleDeactivate = async () => {
        ...
        try {
            setLoading(true)
            const response = await api.delete(`/super-admin/users/PROJECT_MANAGER/${id}`)

            if (response.data.success) {
                alert("Project Manager deactivated successfully!")
                router.push("/super-admin/project-managers")
            } else {
                setError(response.data.message || "Failed to deactivate project manager")
            }
        } catch (err: any) {
    ...
    const handlePermanentDelete = async () => {
        ...
        try {
            setLoading(true)
            const response = await api.delete(`/super-admin/users/PROJECT_MANAGER/${id}/permanent`)

            if (response.data.success) {
                alert("Project Manager permanently deleted!")
                router.push("/super-admin/project-managers")
            } else {
                setError(response.data.message || "Failed to delete project manager")
            }
        } catch (err: any) {
            setError(err.response?.data?.message || "Failed to delete project manager")
        } finally {
            setLoading(false)
        }
    }
    ...
    if (!pm) {
        return (
            <div className="max-w-3xl mx-auto">
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    <p className="text-center text-red-500">Project Manager not found</p>
                </div>
            </div>
        )
    }
                ...
                <div className="mb-6 p-4 bg-gray-50 rounded-lg space-y-2">
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <p className="font-medium text-gray-900">{new Date(pm.createdAt).toLocaleDateString()}</p>
                        </div>
                        <div>
                            <p className="text-sm text-gray-500">Last Login</p>
                            <p className="font-medium text-gray-900">{pm.lastLoginAt ? new Date(pm.lastLoginAt).toLocaleString() : "Never"}</p>
                        </div>
                        <div>
                            <p className="text-sm text-gray-500">Email Verified</p>
                            <p className="font-medium text-gray-900">{pm.emailVerified ? "Yes" : "No"}</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="border-t pt-6">
                        <div className="flex gap-3">
                            <button
                                type="submit"
                                disabled={loading}
                                className="flex-1 px-4 py-2 bg-gradient-to-r from-emerald-600 to-emerald-700 text-white rounded-lg hover:from-emerald-700 hover:to-emerald-800 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                            >
                                {loading ? "Updating..." : "Update Project Manager"}
                            </button>
                            <Link
                                href="/super-admin/project-managers"
                ...
                <div className="mt-8 border-t pt-6">
                    ...
                    <div className="space-y-3">
                        <div className="flex items-center justify-between p-4 border border-yellow-200 bg-yellow-50 rounded-lg">
                            <div>
                                <p className="font-medium text-gray-900">Deactivate Project Manager</p>
                        ...
                        <div className="flex items-center justify-between p-4 border border-red-200 bg-red-50 rounded-lg">
                            <div>
                                <p className="font-medium text-gray-900">Permanently Delete Project Manager</p>
