...
export default function NewServicePage() {
  ...
  const fetchUsers = async () => {
    try {
      const response = await api.get("/ca/users?role=USER")
      if (response.data.success) {
  ...
  const onSubmit = async (data: ServiceFormData) => {
    ...
    try {
      ...
      if (response.data.success) {
        router.push(`/ca/services/${response.data.data.id}`)
      } else {
      ...
      <Card>
        ...
        <CardContent>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            {/* Internal Notes */}
            <div>
              <Label htmlFor="notes">Internal Notes (CA only)</Label>
              <Textarea
                id="notes"
                rows={3}
                placeholder="Internal notes visible only to CA..."
                {...register("notes")}
              />
            </div>
            ...
            <div className="flex gap-4 pt-4">
              ...
              <Button type="button" variant="outline" asChild>
                <Link href="/ca/services">Cancel</Link>
              </Button>
            </div>
          </form>
        </CardContent>
