...
// GET /api/super-admin/documents/hierarchy - Get ALL documents organized by role hierarchy
router.get('/documents/hierarchy', authenticate, requireSuperAdmin, async (req: AuthenticatedRequest, res): Promise<void> => {
    try {
        ...
        const [superAdmin, admins, projectManagers, teamMembers, clients, allDocuments] = await Promise.all([
            // Get super admin info
            superAdminId ? prisma.superAdmin.findFirst({
            ...
            // Get all documents (including soft deleted for super admin)
        ...
        const roleDocuments = {
            self: [] as any[],        // Super Admin's own documents
        ...
        allDocuments.forEach((doc: any) => {
            ...
            // Check if this is a self-document (uploaded by Super Admin)
            if (doc.uploadedByRole === 'SUPER_ADMIN' && doc.uploadedById === superAdminId) {
                roleDocuments.self.push(docData);
            }
            // Check if uploaded by an Admin
            else if (doc.uploadedByRole === 'ADMIN' && doc.uploadedById && adminMap.has(doc.uploadedById)) {
                if (!roleDocuments.admins.has(doc.uploadedById)) {
                    roleDocuments.admins.set(doc.uploadedById, []);
                }
                roleDocuments.admins.get(doc.uploadedById)!.push(docData);
            }
        ...
        const response = {
            selfDocuments: {
                ...
                users: roleDocuments.self.length > 0 ? [{
                    userId: superAdminId,
                    userName: superAdmin?.name || 'Super Admin (You)',
                    userEmail: superAdmin?.email || req.user?.email || '',
                    documentTypes: groupByDocType(roleDocuments.self),
                }] : [],
                totalFiles: roleDocuments.self.length,
            },
            adminDocuments: {
                title: 'Admins Documents',
                users: Array.from(roleDocuments.admins.entries()).map(([adminId, docs]) => {
                ...
                totalFiles: Array.from(roleDocuments.admins.values()).reduce((sum, docs) => sum + docs.length, 0),
            },
...
// GET /api/super-admin/documents - List all documents in the firm
router.get('/documents', authenticate, requireSuperAdmin, async (req: AuthenticatedRequest, res): Promise<void> => {
    try {
        const firmId = req.user?.firmId;
...
// GET /api/super-admin/documents/:id - Get single document details
router.get('/documents/:id', authenticate, requireSuperAdmin, async (req: AuthenticatedRequest, res): Promise<void> => {
    try {
...
// DELETE /api/super-admin/documents/:id - Soft delete document
router.delete('/documents/:id', authenticate, requireSuperAdmin, async (req: AuthenticatedRequest, res): Promise<void> => {
    try {
        const id = String(req.params.id);
...
// DELETE /api/super-admin/documents/:id/permanent - HARD DELETE document (Super Admin ONLY)
router.delete('/documents/:id/permanent', authenticate, requireSuperAdmin, async (req: AuthenticatedRequest, res): Promise<void> => {
    try {
        const id = String(req.params.id);
        const firmId = req.user?.firmId;
        ...
        res.json({
            success: true,
            message: 'Document permanently deleted from database and storage',
        });
    } catch (error) {
...
// GET /api/super-admin/documents/:id/download - Download document file
router.get('/documents/:id/download', authenticate, requireSuperAdmin, async (req: AuthenticatedRequest, res): Promise<void> => {
    try {
        const id = String(req.params.id);
        const firmId = req.user?.firmId;
...
// POST /api/super-admin/documents/upload - Upload a document
router.post('/documents/upload', authenticate, requireSuperAdmin, upload.single('file'), async (req: AuthenticatedRequest, res): Promise<void> => {
    try {
        const userId = req.user?.userId;
        const firmId = req.user?.firmId;
...
// POST /api/super-admin/documents/upload-self - Upload a self-document (Super Admin's own document)
router.post('/documents/upload-self', authenticate, requireSuperAdmin, upload.single('file'), async (req: AuthenticatedRequest, res): Promise<void> => {
    try {
        const userId = req.user?.userId;
        const firmId = req.user?.firmId;
        ...
        // Save file to storage in super-admin-specific folder
        await ensureUploadDirectories();
        const uploadDir = path.join(process.cwd(), 'uploads', 'super-admin', userId);
        await fs.mkdir(uploadDir, { recursive: true });
